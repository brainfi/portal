...
const initialPartidas: Partida[] = [
  { id:1, categoria:'Ventas directas',       tipo:'ingreso', planAnual:840000, planMensual:[65000,65000,68000,70000,70200,72000,72000,68000,74000,76000,78000,80000], real:[63200,66800,74000,88000,68000,0,0,0,0,0,0,0] },
  { id:2, categoria:'Servicios recurrentes', tipo:'ingreso', planAnual:96000,  planMensual:[8000,8000,8000,8000,8000,8000,8000,8000,8000,8000,8000,8000],             real:[8100,8000,8200,8100,7900,0,0,0,0,0,0,0] },
  { id:3, categoria:'Licencias',             tipo:'ingreso', planAnual:60000,  planMensual:[4000,4000,5000,5000,5500,5500,5500,5000,5000,5500,5500,5500],             real:[5200,5800,8100,9200,10200,0,0,0,0,0,0,0] },
  { id:4, categoria:'Nóminas y SS',          tipo:'gasto',   planAnual:196800, planMensual:[16400,16400,16400,16400,16400,16400,16400,16400,16400,16400,16400,16400], real:[16400,16400,16800,17200,18400,0,0,0,0,0,0,0] },
  { id:5, categoria:'Alquiler oficina',      tipo:'gasto',   planAnual:25200,  planMensual:[2100,2100,2100,2100,2100,2100,2100,2100,2100,2100,2100,2100],             real:[2100,2100,2100,2100,2100,0,0,0,0,0,0,0] },
  { id:6, categoria:'Marketing',             tipo:'gasto',   planAnual:36000,  planMensual:[3000,3000,3000,3000,3000,3000,3000,3000,3000,3000,3000,3000],             real:[2800,3100,3400,3900,4300,0,0,0,0,0,0,0] },
  { id:7, categoria:'Software',              tipo:'gasto',   planAnual:10800,  planMensual:[900,900,900,900,900,900,900,900,900,900,900,900],                         real:[740,740,740,740,740,0,0,0,0,0,0,0] },
  { id:8, categoria:'Viajes y dietas',       tipo:'gasto',   planAnual:8400,   planMensual:[600,600,800,800,800,800,800,600,800,800,600,400],                         real:[520,480,610,650,520,0,0,0,0,0,0,0] },
]
...
function DeltaBadge({ real, plan, tipo }: { real: number; plan: number; tipo: TipoPartida }) {
  ...
  return (
    <span style={{ fontSize:11, fontWeight:600, padding:'3px 9px', borderRadius:99, whiteSpace:'nowrap',
...
function EstadoBadge({ real, plan, tipo }: { real: number; plan: number; tipo: TipoPartida }) {
  ...
  return (
    <span style={{ fontSize:10, fontWeight:600, padding:'3px 8px', borderRadius:6,
      background: isGood ? '#EEF1FD' : '#FEF2F2', color: isGood ? '#4361EE' : '#b91c1c' }}>
...
export default function Presupuesto() {
  ...
  const mesaConReal = mesesConReal.length > 0
  ...
  const bajoPlan = partidas.filter(p => {
    const d = calcDelta(sumPeriodo(p.real, mesesConReal), sumPeriodo(p.planMensual, meses))
    return d !== null && p.tipo === 'ingreso' && d < -5
  }).length
  ...
  function handleAddPartida() {
    ...
    setPartidas(prev => [...prev, {
      ...
      planAnual: anual, planMensual: Array(12).fill(Math.round(anual / 12)), real: Array(12).fill(0),
    }])
    setNuevaCategoria('')
    setNuevaImporte('')
    setModalNueva(false)
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', minWidth:640 }}>
              ...
              {arr.map(p => {
                ...
                return (
                  <tr key={p.id} style={{ borderBottom:'1px solid #F4F5F7' }}
                    ...
                    <td style={{ ...td, textAlign:'left', paddingLeft:0, fontWeight:500 }}>{p.categoria}</td>
            ...
            <tfoot>
              <tr style={{ borderTop:'1px solid #ECEEF3' }}>
                <td style={{ ...td, textAlign:'left', paddingLeft:0, fontWeight:700 }}>Total {tipo === 'ingreso' ? 'ingresos' : 'gastos'}</td>
                <td style={{ ...td, fontWeight:700, color:'#888' }}>{fmt(tipo === 'ingreso' ? ingresosPlan : gastosPlan)}</td>
                <td style={{ ...td, color:'#888' }}>{fmt(totalPlan)}</td>
                {mesaConReal && <td style={{ ...td, fontWeight:700, color:colorAcc }}>{totalReal > 0 ? fmt(totalReal) : '—'}</td>}
                <td style={td}>{mesaConReal && totalReal > 0 && <DeltaBadge real={totalReal} plan={totalPlan} tipo={tipo} />}</td>
                <td /><td />
              </tr>
            </tfoot>
          </table>
  ...
  const cumplimientoPct = ingresosPlan > 0 ? Math.round((ingresosReal / ingresosPlan) * 100) : 0
    ...
    <Layout title="Presupuesto">
      ...
      <div style={{ background:'#4361EE', borderRadius:16, padding:'20px 24px', position:'relative' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:16, flexWrap:'wrap' }}>
          <div>
            <div style={{ fontSize:9, fontWeight:700, color:'rgba(255,255,255,0.55)', textTransform:'uppercase', letterSpacing:'0.12em', marginBottom:4 }}>Presupuesto 2026</div>
            <div style={{ fontSize:20, fontWeight:700, color:'#fff', letterSpacing:'-0.3px', marginBottom:10 }}>Plan anual vs ejecución</div>
            <div style={{ display:'flex', alignItems:'center', gap:12 }}>
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                <div style={{ width:140, height:5, background:'rgba(255,255,255,0.2)', borderRadius:99, overflow:'hidden' }}>
                  <div style={{ width:`${Math.min(cumplimientoPct,100)}%`, height:'100%', background:'#fff', borderRadius:99 }} />
                </div>
                <span style={{ fontSize:13, fontWeight:700, color:'#fff' }}>{cumplimientoPct}%</span>
              </div>
              <span style={{ fontSize:11, color:'rgba(255,255,255,0.55)' }}>cumplimiento · {periodoActivo.label}</span>
            </div>
          </div>
      ...
      {/* ── KPI cards — igual que Dashboard ── */}
      <div className="pres-kgrid" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12 }}>
        {[
          { lbl:'Ingresos · plan', val:fmtK(ingresosPlan), real:ingresosReal, plan:ingresosPlan, tipo:'ingreso' as TipoPartida, iconBg:'#F0F9F4', iconColor:'#2DC653', icon:'ti-arrow-up-right' },
          { lbl:'Gastos · plan',   val:fmtK(gastosPlan),   real:gastosReal,   plan:gastosPlan,   tipo:'gasto'   as TipoPartida, iconBg:'#FEF2F2', iconColor:'#EF4444', icon:'ti-arrow-down-right' },
          { lbl:'Utilidad neta',   val:fmtK(utilPlan),     real:utilReal,     plan:utilPlan,     tipo:'ingreso' as TipoPartida, iconBg:'#FFF8E6', iconColor:'#F4A100', icon:'ti-chart-line' },
          { lbl:'Alertas', val:`${sobrePlan + bajoPlan}`, esAlertas:true, iconBg:sobrePlan > 0 ? '#FEF2F2' : '#F4F5F7', iconColor:sobrePlan > 0 ? '#EF4444' : '#B0B7C3', icon:'ti-bell' },
        ].map((k, i) => (
          <div key={i} style={{ ...card, padding:'18px 20px' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
              ...
              <div style={{ width:30, height:30, borderRadius:8, background:(k as any).iconBg, display:'flex', alignItems:'center', justifyContent:'center' }}>
                <i className={`ti ${(k as any).icon}`} style={{ fontSize:15, color:(k as any).iconColor }} aria-hidden="true" />
              </div>
            </div>
            <div style={{ fontSize:24, fontWeight:400, color:'#1a1a1a', letterSpacing:'-0.5px', marginBottom:6 }}>{k.val}</div>
            {(k as any).esAlertas ? (
              <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                <span style={{ fontSize:11, fontWeight:600, padding:'2px 8px', borderRadius:99, background:sobrePlan > 0 ? '#FEF2F2' : '#F4F5F7', color:sobrePlan > 0 ? '#b91c1c' : '#888' }}>{sobrePlan} sobre</span>
                <span style={{ fontSize:11, fontWeight:600, padding:'2px 8px', borderRadius:99, background:bajoPlan > 0 ? '#FFF8E6' : '#F4F5F7', color:bajoPlan > 0 ? '#92400E' : '#888' }}>{bajoPlan} debajo</span>
              </div>
            ) : (
              <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                {(k as any).real > 0 && <DeltaBadge real={(k as any).real} plan={(k as any).plan} tipo={(k as any).tipo} />}
                {(k as any).real > 0 && <span style={{ fontSize:11, color:'#B0B7C3' }}>real {fmtK((k as any).real)}</span>}
              </div>
            )}
          </div>
        ))}
      ...
      {/* ── Gráfico barras real + línea forecast ── */}
      <div style={card}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:16 }}>
          ...
          <div style={{ display:'flex', gap:14, alignItems:'center' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5 }}>
              <div style={{ width:18, height:0, borderTop:'2px dashed #C7D2F8' }} />
